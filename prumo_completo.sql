...
-- =====================================================================
--  v13 — "Monte sua casa": leads da página pública de captação
-- =====================================================================
create table if not exists public.leads_projeto (
  id uuid primary key default gen_random_uuid(),
  nome text, contato text, cidade text,
  tipo text, padrao text, telhado text, modelo text, obs text,
...
create policy "leads: equipe vê"      on public.leads_projeto for select using ( public.eh_equipe() );
create policy "leads: equipe gerencia" on public.leads_projeto for update using ( public.eh_equipe() ) with check ( public.eh_equipe() );
