...
/* =========================================================================
 *  PORTFÓLIO DE ACABAMENTOS — página pública (sem login)
 *  Catálogo de materiais MS por categoria, cada uma com 3 opções.
 *  Imagens em /materiais/<slug>.jpg (recortadas do catálogo).
 * ======================================================================= */
export default function Portfolio() {
  ...
  const porGrupo = useMemo(() => {
    const g = {}
    lista.forEach((m) => { (g[m.grupo] = g[m.grupo] || []).push(m) })
    return g
  }, [lista])
    ...
    <>{header}
      <div className="content" style={{ maxWidth: 1120, margin: '0 auto' }}>
        <div className="pg-head" style={{ textAlign: 'center' }}>
          ...
          <div className="pg-sub" style={{ maxWidth: 620, margin: '8px auto 0' }}>
            As opções que você escolhe pra dar cara à sua casa, dos revestimentos ao paisagismo. Cada categoria com 3 padrões. Clique numa imagem para ampliar.
          </div>
        </div>

        {/* filtros */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'center', margin: '16px 0 22px' }}>
          <div style={{ maxWidth: 340, width: '100%' }}>
            <input style={inp} placeholder="Buscar (ex.: porcelanato, alumínio, mármore…)" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
        ...
        {lista.length === 0 && <div className="muted" style={{ textAlign: 'center', fontSize: 13 }}>Nada encontrado. Tente outro termo.</div>}
        ...
        <div className="center-note" style={{ margin: '22px auto', fontSize: 12 }}>
          MS Construções Inteligentes · Os acabamentos definem o padrão e o valor final da obra
        </div>
      </div>
